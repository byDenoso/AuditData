...
def angular_mask_test(rra,rde,rz):
 m=(rz>Z0-.0125)&(rz<Z0+.0125); U=unitvec(rra[m],rde[m]); tr=cKDTree(U)
 theta=75/dc(Z0); chord=2*np.sin(theta/2); u0=unitvec([RA0],[DEC0])[0]
 obs=tr.query_ball_point(u0,chord,return_length=True)
 idx=RNG.choice(len(U),min(1500,len(U)),replace=False); vals=tr.query_ball_point(U[idx],chord,return_length=True)
...
def fine_slice_random_centers(ra,de,z,rra,rde,rz,label):
 m=(z>Z0-.0125)&(z<Z0+.0125); U=unitvec(ra[m],de[m]); tr=cKDTree(U)
 rm=(rz>Z0-.0125)&(rz<Z0+.0125); RU=unitvec(rra[rm],rde[rm]);
 theta=75/dc(Z0); chord=2*np.sin(theta/2); u0=unitvec([RA0],[DEC0])[0]
 obs=tr.query_ball_point(u0,chord,return_length=True)
 idx=RNG.choice(len(RU),min(3000,len(RU)),replace=False); centers=RU[idx]
 # exclude controls overlapping candidate aperture
 keep=np.linalg.norm(centers-u0,axis=1)>2*chord;centers=centers[keep]
 vals=tr.query_ball_point(centers,chord,return_length=True)
 return dict(tracer=label,objects=int(len(U)),random_centers=int(len(vals)),obs=int(obs),mean=float(np.mean(vals)),median=float(np.median(vals)),ratio_to_mean=float(obs/max(np.mean(vals),1)),zscore=float((obs-np.mean(vals))/max(np.std(vals,ddof=1),1)),empirical_p=float((np.sum(vals<=obs)+1)/(len(vals)+1)),q05=float(np.quantile(vals,.05)),q95=float(np.quantile(vals,.95)))
...
def main():
 ...
 E=pd.DataFrame(Erows);L.to_csv(o/'lrg_profiles.csv',index=False);E.to_csv(o/'elg_profiles.csv',index=False)
 ...
 mask=angular_mask_test(rra,rde,rz); fine=[fine_slice_random_centers(lra,lde,lz,rra,rde,rz,'LRG'),fine_slice_random_centers(era,ede,ez,rra,rde,rz,'ELG')]
 outer=L[L.R_mpc>=90]; comp=float(outer.shell_ratio.max()); rcomp=float(outer.loc[outer.shell_ratio.idxmax(),'R_mpc']); basin=float((cg.ratio<.8).mean()); minrow=cg.loc[cg.ratio.idxmin()].to_dict()
 summary={'candidate':'NEXO-V14','ra':RA0,'dec':DEC0,'z':Z0,'R_nominal_mpc':75,'lrg_objects':len(lz),'elg_objects':len(ez),'random_objects':len(rz),'random_alpha':alpha,'mask_test':mask,'fine_slice_random_centers':fine,'compensation_shell_max_ratio':comp,'compensation_shell_R_mpc':rcomp,'center_grid_fraction_ratio_lt_0p8':basin,'center_grid_min':minrow,'lrg_ratio_75':float(L[L.R_mpc==75].ratio.iloc[0]),'lrg_weighted_ratio_75':float(L[L.R_mpc==75].weighted_ratio.iloc[0])}
 (o/'summary.json').write_text(json.dumps(summary,indent=2,default=float));print(json.dumps(summary,indent=2,default=float));print('LRG PROFILE');print(L.to_string(index=False));print('FINE',fine);print('MASK',mask)
