...
def run_shoes_gate(a,out):
    ...
    for k in range(1,6):
        ...
        random_rows.append(dict(k=k,n_random=nrand,median_delta=float(np.median(vals)),p05=float(np.quantile(vals,.05)),p95=float(np.quantile(vals,.95)),p_random_as_low_or_lower=float((1+np.sum(vals<=obs_down))/(nrand+1)),p_random_abs_ge_topabs=float((1+np.sum(np.abs(vals)>=abs(obs_abs)))/(nrand+1)),p_random_abs_ge_full_residual=float((1+np.sum(np.abs(vals)>=residual))/(nrand+1))))
    ...
    for k in range(1,4):
        for combo in itertools.combinations(names,k):
            ...
            q,cov,p=fit_delete_cached(y,L,params,cache,D,())
            h,s,_=get_h0(q,cov,p)
            ar.append(dict(anchors_removed='+'.join(combo),n_rows=len(D),H0=h,sigma_H0=s,delta_H0=h-h0,residual_fraction=residual_fraction(h-h0,residual)))
    ...
    return dict(
      ...
      max_anchor_subset_shift=float(A.delta_H0.abs().max()), max_anchor_subset=str(A.loc[A.delta_H0.abs().idxmax(),'anchors_removed'])
    )
...
def main():
    ...
    verdict={
      ...
      'paper_gates':{
        ...
        'any_anchor_subset_explains_full_residual': bool(shoes['max_anchor_subset_shift']>=shoes['full_residual']),
        'best_downward_k5_reaches_PEERlocal': bool(bestdown.H0<=PEER_LOCAL),
        'best_downward_k5_H0': float(bestdown.H0),
        'random_subset_full_residual_min_p': float(rnd.p_random_abs_ge_full_residual.min()),
      ...
      'caveat':'Random subset tests are structured robustness nulls on the observed ladder, not full end-to-end cosmological mocks. PEER+Local sigma is bookkeeping only.'
    }
