...
async function loadBundle() {
  bundlePromise ||= (async () => {
    ...
    if (actual !== BUNDLE_SHA256) throw new Error(`bundle SHA divergente: ${actual}`);
...
function inferSchema(manifest) {
  if (['peer-studio/manifest-v1', 'peer-studio/canonical-snapshot-v1'].includes(manifest?.schema)) return manifest.schema;
  if (!manifest?.schema && manifest?.version && manifest?.generated_at && Array.isArray(manifest?.sources) && Array.isArray(manifest?.series) && manifest?.detection_status && manifest?.cosmology) {
...
function validateManifest(manifest) {
  ...
  for (const source of manifest?.sources || []) {
    ...
    if (source?.sha256 !== undefined && !/^[a-f0-9]{64}$/i.test(String(source.sha256))) errors.push(`SHA-256 inválido em ${source?.id || 'source'}`);
  }
  for (const series of manifest?.series || []) {
    ...
    for (const sourceId of series?.source_ids || []) if (!sourceIds.has(sourceId)) errors.push(`source_id quebrado: ${sourceId}`);
    if (Array.isArray(series?.x) && Array.isArray(series?.y) && series.x.length !== series.y.length) errors.push(`x/y inconsistente: ${series?.id || 'series'}`);
  }
...
async function readManifest(url, source, expectedSha) {
  ...
  if (expectedSha && sha256.toLowerCase() !== expectedSha.toLowerCase()) throw new Error(`${source} SHA-256 divergente: ${sha256}`);
...
function emitManifest(res, snapshot, stale, reason = '') {
  ...
  res.setHeader('access-control-expose-headers', 'x-peer-manifest-schema, x-peer-manifest-sha256, x-peer-snapshot-source, x-peer-snapshot-stale, x-peer-snapshot-generated-at, x-peer-snapshot-reason');
  securityHeaders(res);
